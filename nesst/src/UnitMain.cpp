...
const unsigned char bgPalDefault[4*16]={
...
AnsiString save_data(const char *name,unsigned char *src,int size,int filter)
{
	...
	if(rle)
	{
		...
		if(size<0) return "No unused values found (tiles and/or attributes), can't be saved as RLE due to limitations of the format.";
	}
	else
	{
		dst=src;
...
bool __fastcall TFormMain::LoadSessionText(AnsiString filename)
{
	...
	MPutSelectionToMetaspriteSkipZero->Checked=nss_get_int(text,size,"MenuMetaSprSkipZero=")?true:false;
...
void __fastcall TFormMain::FormCreate(TObject *Sender)
{
	...
	memcpy(bgPal,bgPalDefault,sizeof(bgPal));
...
void __fastcall TFormMain::ChangeNameTableFrame(int dir)
{
	...
	if(OpenNameTable(namestr))
	{
		...
		save_data((RemoveExt(OpenDialogName->FileName)+".nam").c_str(),buf,name_size()+attr_size(),SAVE_NAMETABLE_NAM);
...
void __fastcall TFormMain::MPutSelectionToMetaspriteSkipZeroClick(
TObject *Sender)
{
	MPutSelectionToMetaspriteSkipZero->Checked^=true;	
}
//---------------------------------------------------------------------------
...
void __fastcall TFormMain::MAddOffsetMetaSpritesClick(TObject *Sender)
{
	int i,j,off;
