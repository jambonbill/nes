...
void eraseAll(void){//clear screen
	...
	while(i<29)
	{
		...
		vram_adr(NTADR_A(0,i));// sets a start address
		for(j=0;j<32;j++){
			vram_put(0x20);//petscii space
			//list[0]=MSB(NTADR_A(j,i));//compute nametable adress MSB
			//list[1]=LSB(NTADR_A(j,i));//compute nametable adress LSB
...
void main(void)
{
	...
	while(1)
	{
		...
		if(i&PAD_RIGHT&&x<31) x+=1;
		if(i&PAD_UP   &&y>  0) y-=1;
		if(i&PAD_DOWN &&y<29) y+=1;
		if(i&PAD_START){
			//sprnum=rand8();
			//bgClearA();//break
			eraseAll();
